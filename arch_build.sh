...
export KERNEL_DEFCONFIG=sdm670-perf_defconfig
export SETUP_KERNELSU=true  # Enable if you want KernelSU
export KernelSU_TAG=main    # Select KernelSU tag or branch
export KernelSU_LKM=false    # Build KernelSU as a kernel module
export KernelSU_SUSFS=true  # Enable SUSFS
...
setup_kernelsu() {
    ...
    if [ "$KernelSU_LKM" == true ]; then
        scripts/config --file "arch/$ARCH/configs/$KERNEL_DEFCONFIG" --set-val CONFIG_KSU m
        scripts/config --file "arch/$ARCH/configs/$KERNEL_DEFCONFIG" -d MODULE_SIG -d MODULE_SIG_FORCE -d MODULE_SIG_ALL
        ast-grep -U -p '$$$ check_exports($$$) {$$$}' -r '' scripts/mod/modpost.c
        ast-grep -U -p 'err |= check_exports(mod);' -r '' scripts/mod/modpost.c
        scripts/config --file "arch/$ARCH/configs/$KERNEL_DEFCONFIG" -d KSU_SUSFS
    else
        scripts/config --file "arch/$ARCH/configs/$KERNEL_DEFCONFIG" --set-val CONFIG_KSU y
        test -d KernelSU && rm -rf KernelSU
        if [ "$KernelSU_SUSFS" == true ]; then
            git clone -b main https://github.com/natsumerinchan/KernelSU.git
            scripts/config --file "arch/$ARCH/configs/$KERNEL_DEFCONFIG" --set-val CONFIG_KSU_SUSFS y
        else
            scripts/config --file "arch/$ARCH/configs/$KERNEL_DEFCONFIG" -d KSU_SUSFS
        fi
...
make_anykernel3_zip() {
    ...
    git clone https://github.com/shravansayz/AnyKernel3 --depth=1 $KERNEL_PATH/AnyKernel3
